...
pub enum QualityFlag {
    ...
    Substituted,
    /// Calculated / derived value (Rechenwert).
    ///
    /// Derived from other meter readings (e.g. Residuallast = Bezug −
    /// Einspeisung) rather than read off a register. Not a
    /// [`Preliminary`](Self::Preliminary) value, which is a *measured* one that
...
impl QualityFlag {
    ...
    ///
    /// The `QTY` Mengen-Qualifier of EDI@Energy **MSCONS MIG 2.4c** (binding
    /// since 03.04.2024; MIG 2.5, binding 01.10.2026, carries the same list):
    ///
    ...
    ///
    /// The other three have **no qualifier**, and that is the useful part of
    /// the answer:
    ///
    /// - a [`Calculated`](Self::Calculated) value is a *Rechenwert*, which the
    ///   market expresses as the Plausibilisierungshinweis `STS+Z33 ZR5` beside
    ///   an ordinary quantity, not as a quantity qualifier of its own;
    /// - a [`Corrected`](Self::Corrected) value is an ordinary value carrying a
    ///   `STS+Z34 Korrekturgrund`, so its qualifier is that of the value it
    ///   replaces;
    /// - [`Unknown`](Self::Unknown) is this crate's admission that the status
    ///   did not arrive. The market has no code for it — `Z30 Fehlender Wert`
    ///   says the *value* is missing, which is a different statement — so a
    ///   caller must resolve it before writing a message rather than picking
    ///   the nearest-looking code.
    ///
    /// ```rust
    /// use metering::QualityFlag;
    ///
...
impl MeterInterval {
    ...
    /// A measured interval with no OBIS code.
    ///
    /// The common shape, and the one that says what it claims: the quality is
    /// [`Measured`](QualityFlag::Measured) because the constructor is named
    /// for it, rather than because a `Default` chose it. Anything else is a
    /// builder step, so an estimated or substituted value cannot be created by
    /// forgetting a field.
    ///
    /// ```rust
    ...
    ///
    /// let iv = MeterInterval::measured(
    ///     datetime!(2026-06-01 12:00 UTC),
    ///     datetime!(2026-06-01 12:15 UTC),
    ///     dec!(2.5),
    /// );
    /// assert_eq!(iv.quality, QualityFlag::Measured);
    /// assert_eq!(iv.demand_kw(), Some(dec!(10)));
    /// ```
    #[must_use]
    pub const fn measured(from: OffsetDateTime, to: OffsetDateTime, value: Decimal) -> Self {
        Self {
            from,
            to,
            value,
    ...
    /// A measured quarter-hour starting at `from` — the German settlement slot.
    ///
    /// The end is `from + 15 min`, which is exact in UTC at every date: a
    /// quarter-hour is a **fixed** length, and only the *day* changes length at
    /// a DST transition ([`crate::calendar`]).
    ///
    /// ```rust
    ...
    /// use time::macros::datetime;
    ///
    /// let slot = MeterInterval::quarter_hour(datetime!(2026-06-01 12:00 UTC), dec!(2.5));
    /// assert_eq!(slot.to, datetime!(2026-06-01 12:15 UTC));
    /// ```
    #[must_use]
    pub fn quarter_hour(from: OffsetDateTime, value: Decimal) -> Self {
        Self::measured(from, from + time::Duration::minutes(15), value)
    }

    /// A measured hour starting at `from` — the German gas settlement slot.
    #[must_use]
    pub fn hour(from: OffsetDateTime, value: Decimal) -> Self {
        Self::measured(from, from + time::Duration::hours(1), value)
    }

    /// The same interval with a different quality (builder style).
    #[must_use]
    ...
    /// The same interval on a named OBIS channel (builder style).
    ///
    /// Stamping the channel is what makes [`direction`](Self::direction),
    /// [`crate::aggregation::sum_by_direction`] and the register-aware parts of
    /// [`crate::reading`] able to answer at all.
    #[must_use]
    ...
    pub fn demand_kw(&self) -> Option<Decimal> {
        ...
        // `value × 3600 ÷ secs`, not `value ÷ (secs ÷ 3600)`: the second form
        // divides twice, and the first of those two quotients does not
        // terminate for any second count that is not a divisor of 3 600. One
        // rounding, at the end — the rule the whole crate follows.
        Some(self.value * Decimal::from(3600u32) / Decimal::from(secs))
    }
    ...
    ///
    /// ```rust
    /// use metering::{Direction, MeterInterval, ObisCode};
    /// use rust_decimal::dec;
    /// use time::macros::datetime;
    ///
    /// let iv = MeterInterval::quarter_hour(datetime!(2026-01-01 0:00 UTC), dec!(2.5))
...
mod tests {
    ...
    /// The constructors say what they claim: a named `measured` rather than a
    /// `new` that picks a quality nobody asked for. `Unknown` — the `Default` —
    /// is reachable only by asking for it.
    #[test]
    fn the_constructors_claim_only_what_they_name() {
        let iv = MeterInterval::measured(
            datetime!(2026-06-01 12:00 UTC),
            datetime!(2026-06-01 12:15 UTC),
            dec!(2.5),
        ...
        assert_eq!(iv.quality, QualityFlag::Measured);
        assert_eq!(iv.obis_code, None);
        assert_eq!(
            iv.clone().with_quality(QualityFlag::Substituted).quality,
            QualityFlag::Substituted
        );
        assert_eq!(
            iv.with_obis(ObisCode::STROM_BEZUG_TOTAL).direction(),
            Some(Direction::Import)
        );
    }

    /// A quarter-hour is a fixed length: only the *day* changes length at a
    /// DST transition, so `from + 15 min` is right on the transition day too.
    #[test]
    fn a_quarter_hour_is_fifteen_minutes_even_across_a_transition() {
        // 02:00 local on the October Sunday is 00:00 UTC, inside the repeated
        // hour; the slot is still 900 s.
        let slot = MeterInterval::quarter_hour(datetime!(2026-10-25 0:00 UTC), dec!(1));
        assert_eq!(slot.duration_secs(), 900);
        assert_eq!(slot.to, datetime!(2026-10-25 0:15 UTC));
        assert_eq!(
            MeterInterval::hour(datetime!(2026-10-25 0:00 UTC), dec!(1)).duration_secs(),
            3600
        );
    }
